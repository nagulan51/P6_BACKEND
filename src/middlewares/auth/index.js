...
const needTokenJsonBody = { message: "Un jeton valide est nécessaire pour accéder a cet route." };
...
export const auth = async (req, res, next) => {
    ...
    try {
        ...
        if (!user) return res.status(403).json({ message: "Il y a aucun utilisateur associé a ce jeton." });

        req.user = user;
        next();
    } catch (error) {
        if (error instanceof MongooseError) {
            res.status(500).json({ message: "Impossible d'accéder a l'utilisateur dans la base de donnée" });
